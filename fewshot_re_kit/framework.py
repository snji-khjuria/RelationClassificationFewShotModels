...
class FewShotREModel(nn.Module):
    ...
    def accuracy(self, pred, label):
        ...
        '''
        return torch.sum((pred.view(-1) == label.view(-1)).type(torch.FloatTensor))
...
class FewShotREFramework:
    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            iter_sample += 1
            sys.stdout.write('step: {0:4} | loss: {1:2.6f} '.format(it + 1, iter_loss / iter_sample) + '\r')
            sys.stdout.flush()
            ...
            if (it + 1) % val_step == 0:
                train_acc = self.do_eval(model, B, N_for_eval, K, Q, val_iter, 0)
                acc = self.do_eval(model, B, N_for_eval, K, Q, val_iter, 1)
                test_acc = self.do_eval(model, B, 16, K, Q, val_iter, 2)
    ...
    def do_eval(self, model, B, N, K, Q, eval_iter, isVal=0, ckpt=None):
        '''
        model: a FewShotREModel instance
        B: Batch size
        N: Num of classes for each batch
        ...
        for it in range(eval_iter):
            ...
            right = model.accuracy(pred, label)
            iter_right += self.item(right.data)
            iter_total = label.size(0)*label.size(1)
            iter_sample += 1
            correct+=self.item(right.data)
            total+=label.size(0)*label.size(1)


            sys.stdout.write(
                '[EVAL] step: {0:4} | accuracy: {1:3.2f}%'.format(it + 1, 100 * correct / total) + '\r')
            sys.stdout.flush()
        print("")
